  ...
  <el-container>
    ...
    <el-container>
      ...
      <el-aside width="228px" height="100%">
        <v-menu :selected1="selected" :hidden="hidden" index="palace1"></v-menu>
      </el-aside>
      ...
      <el-dialog title="新增分类" :visible.sync="dialogFormVisible">
        <div>
          分类名称
          <el-input v-model="name" auto-complete="off" placeholder="请输入分类介绍（不超过12个字）" class="new_input"></el-input>
        </div>
        <div class="selectBox">
          分类介绍
      ...
      <el-dialog title="编辑分类" :visible.sync="dialogFormVisible1">
        <div>
          分类名称
          <el-input v-model="name1" auto-complete="off" placeholder="请输入分类介绍（不超过12个字）" class="new_input"></el-input>
        </div>
        <div class="selectBox">
          分类介绍
...
//        console.log(`当前页: ${val}`);
      ...
      search:function () {
        ...
        if(this.typename==''){
          ...
          this.$http('post','/Front/Category/seachCategory',{name:this.typename,type:1,team_id:this.team_id}).then(function (res) {
//            console.log(res);
      ...
      //查看
      see:function (row) {
        this.$router.push({name:'palace',params:{id:row.cid}});
      },
      //新增
      add(){
...
//      请求数据列表
      init(){
        let that=this;
        this.$http('post','/Front/Category/categoryList',{page:this.currentPage,type:1,team_id:this.team_id}).then(function (res) {
